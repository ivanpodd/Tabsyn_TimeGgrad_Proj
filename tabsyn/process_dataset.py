...
def split(transactions, time_q, user_q):
    ...
    print('~~~ user_id min-max:', transactions['user_id'].min(), transactions['user_id'].max())
    print(f'~~~ user_id: quartile - {user_q}, pivot value - {user_val}')
    print('~~~ time_diff min-max:', transactions['time_diff'].min(), transactions['time_diff'].max())
    print(f'~~~ time_diff: quartile - {time_q}, pivot value - {time_val}')
    ...
    print('~~~ train (count) (train/all):', train_n, train_n/all_n)
    print('~~~ test  (count) (test/all):', test_n, test_n/all_n)
    print('~~~ proportion:', train_n/test_n)
    return transactions_train, transactions_test

#added - changed
def train_val_test_split(data_df, info):
    time_q = 0.9 if not info['time_q'] else float(info['time_q'])
    user_q = 0.9 if not info['user_q'] else float(info['user_q'])
    # train, test, seed
    return *split(data_df, time_q, user_q), 0
...
def process_data(name):
    ...
    if info['file_type'] == 'csv':
        ...
        #added
        data_df = calc_dt(data_df)
        print('~~~ New columns:', data_df.columns)
    ...
    if info['test_path']:
        ...
        #added - changed
        train_df, test_df, seed = train_val_test_split(data_df, info)
